...
WForm<-read_excel(file.path(dataOutDir,paste('wesp_FormIn.xlsx.xlsx',sep='')),
...
caseMissing<-c('F12','F13','F16','F18','F20','F21','F24','F25','F26','F27','F28','F29','F30','F31','F32','F33','F34','F35',
               'F36','F37','F38','F39','F40','F44','F50','F52','F41','F43','F53','F55','F59')
df5<-lapply(1:length(caseMissing), function(x) {
  VName0<-paste0(caseMissing[x],'_0')
  VName1<-paste0(caseMissing[x],'_1')
  WForm4.1 %>%
    mutate(!!VName1 := if_else(!!rlang::sym(VName0)=='0','1',as.character(!!rlang::sym(VName1)))) %>%
    dplyr::select(!!rlang::sym(VName0),!!rlang::sym(VName1))
})
df6<-cbind(WForm_Wetland_Co,do.call(cbind,df5)) %>%
  dplyr::select(-c(paste0(caseMissing,'_0')))
WForm4<-WForm4.1 %>%
  dplyr::select(-c(paste0(caseMissing,'_1'))) %>%
