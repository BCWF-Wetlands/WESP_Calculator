...
WForm2 <- cbind(WForm1,(do.call(cbind, df3))) %>%
  replace(is.na(.),0) #%>%
 # mutate(across(.funs=~str_replace(.,'N/A','0')))
...
WForm4.1<-WForm3 %>%
  ...
  dplyr::rename(F23_1 = F23_0) %>%
  dplyr::rename(F22_1 = F22_0) %>%
  dplyr::rename(F42_1 = F42_0) %>%
  dplyr::rename(F49_1 = F49_0)
  #mutate(F2_A0=0) %>%
  #mutate(F2_B0=0) #%>%
...
WForm4Check<-WForm4.1 %>%
  ...
  dplyr::select(Wetland_Co,F22_1,F23_1,F42_1,F49_1,F4_0, c(paste0('F4_',(1:4))),F17_0,, c(paste0('F17_',(1:5))))
...
WriteXLS(WForm4,file.path(dataOutDir,paste('clean_F.xlsx',sep='')),
