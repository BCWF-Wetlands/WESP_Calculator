...
#Modify y/n to 1/0 and
#Fix Wetland_Co 34028 field F45_1 - remove text
WForm4<-WForm3 %>%
  ...
  mutate(across(c(F2,F3,F4,F5,F6,F7,F8,F9,F10,F11,F12, F13,F14,F15,F16,F17,F18,F19,F20,
                  F21,F24,F25,F26,F27,F28,F29,F30,F31,F32,F33,F34,F35,F36,F37,F38,F39,F40,
                  F41,F43,F44,F47,F48,F50,F51,F52,F53,F54,F55,F56,F57,F58,F61),
  ...
  mutate(F45_1=if_else(Wetland_Co=='34028', '6.8', F45_1))

WForm4 %>%
  dplyr::select(Wetland_Co,F3,F4,F5,F6,F7,F8,F9,F10,F57_7, F45_1)
...
WetlandPlotData <- WForm5 %>%
  ...
  mutate(across(c(F1, starts_with('F1_'),F2, starts_with('F2_'),F3, starts_with('F3_'),F4, starts_with('F4_'),
                  F5, starts_with('F5_'),F6, starts_with('F6_'),F7, starts_with('F7_'),F8, starts_with('F8_'),
                  F9, starts_with('F9_'),F10, starts_with('F10_'),F11, starts_with('F11_'),F12, starts_with('F12_'),
                  F13, starts_with('F13_'),F14, starts_with('F14_'),F15, starts_with('F15_'),F16, starts_with('F16_'),
                  F17, starts_with('F17_'),F18, starts_with('F18_'),F19, starts_with('F19_'),F20, starts_with('F20_'),
                  ...
                  F53, starts_with('F53_'),F54, starts_with('F54_'),F55, starts_with('F55_'),F56, starts_with('F56_'),
                  F57, starts_with('F57_'),F58, starts_with('F58_'),F59, starts_with('F59_'),F60, starts_with('F60_'),
                  F61,starts_with('F61_')), as.numeric)) %>%
  # mutate(F57_7=if_else(as.numeric(F57_1+F57_2+F57_3+F57_4+F57_5+F57_6)==0, 1, F57_7))
  #mutate(across(c(F57_1,F57_2,F57_3,F57_4,F57_5,F57_6,F57_7), as.numeric)) %>%
  mutate(F57_7=if_else((F57_1+F57_2+F57_3+F57_4+F57_5+F57_6)==0, 1,
                       if_else(is.na(F57_7), 0, F57_7)))

WetlandPlotData[is.na(WetlandPlotData)] <- 0


WFcheck<-WForm5 %>%
  dplyr::select(Batch_ID, 2,F3,F4,F5,F6,F7,F8,F9,F10,F57_7, F45_1)
